...
class RetirosCliente extends Component
{
    ...
    public function render()
    {
        ...
        $this->retiroscliente = DB::table('movimientos_billeteras')
            ...
            ->select('movimientos_billeteras.*','transacciones.*' ,'users.*')->where('movimientos_billeteras.MONTO_TRANSACCION', 'like', '%'.$this->search.'%')->where('transacciones.TIPO_TRANSACCION', '=','RT')
